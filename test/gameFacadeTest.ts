...
describe("########## Verify the Game Facade ##########", () => {
  ...
  it("Should update Team 1 location", async () => {
    const result = await GameFacade.updateLocation("t1", 20, 30);
    expect(result.lontitude).to.be.equal(20);
    expect(result.latitude).to.be.equal(30);
