...
export default class GameFacade {
  ...
  static async updateLocation(userName: string, lon: Number, lat: Number) {
    GameFacade.isDbReady();
    let user;
    ...
    try {
      ...
      throw new ApiError("wrong username", 403);
    }
    const date = new Date();
    try {
      const found = await positionCollection.findOneAndUpdate(
      ...
      const formatted = {
        ...
        lontitude: found.value.location.coordinates[0],
        latitude: found.value.location.coordinates[1],
      };
      return formatted;
    } catch (err) {
      throw err;
